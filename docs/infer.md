...
Exploration can crash the interpreter or never return, and neither yields a signature.
Both surface as an `InferError` that names where exploration stopped, after at most a
minute.
